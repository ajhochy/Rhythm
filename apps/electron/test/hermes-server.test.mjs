...
function fixture(t, options = {}) {
  const calls = [], snapshots = [], children = [], probes = [];
  ...
  return { supervisor, calls, snapshots, children, probes };
}
...
for (const built of [false, true]) test(`Hermes uses exact pinned serve arguments, built dist=${built}`, async (t) => {
  ...
  assert.deepEqual(f.calls.map(({ binary, args }) => ({ binary, args })), [
    ...
    { binary: '/fixture/hermes', args: ['serve', '--port', '9121', '--host', '127.0.0.1', ...(built ? ['--skip-build'] : [])] },
  ]);
  assert.deepEqual(f.probes, [9121]);
  assert.deepEqual(f.calls[1].settings.stdio, ['ignore', 'pipe', 'pipe']);
});

for (const code of [200, 302, 401, 403, 404, 503]) test(`Hermes HTTP ${code} is listening without redirect following`, async (t) => {
  const f = fixture(t, { fetch: async (url, options) => {
    assert.equal(url, 'http://127.0.0.1:9122'); assert.equal(options.method, 'GET'); assert.equal(options.redirect, 'manual');
    return { status: code };
...
test('Hermes login-shell resolution uses the official fallback and real install directory', async (t) => {
  ...
  await mkdir(join(root, 'web/dist'), { recursive: true }); await writeFile(join(root, 'web/dist/index.html'), 'built');
...
async function mainFixture({ enabled = '1', argv = [] } = {}) {
  const calls = [], windows = [], handlers = new Map(), dialogs = []; let options, listener, releaseStop;
  ...
  return { calls, windows, handlers, app, options, dialogs, event, publish: (value) => listener(value), extraWindow: () => new Window(), release: () => releaseStop() };
}

test('Hermes main IPC authorizes owned document only, broadcasts status, and awaits shutdown', async () => {
  const f = await mainFixture();
  assert.deepEqual(f.calls, ['start-hermes']);
