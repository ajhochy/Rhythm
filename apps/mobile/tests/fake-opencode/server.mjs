...
const applicablePatch = 'diff --git a/src/demo.ts b/src/demo.ts\n--- a/src/demo.ts\n+++ b/src/demo.ts\n@@ -1 +1 @@\n-export const demo = "OpenCode 1.14.49";\n+export const demo = "OpenCode SDK 1.14.49";\n';
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && pathname === '/global/health') {
      sendJson(res, 200, { healthy: true, version: '1.14.49' });
      return;
    }
    ...
    if (req.method === 'POST' && pathname === '/vcs/apply') {
      const body = await readJson(req);
      const expected = 'export const demo = "OpenCode 1.14.49";\n';
      ...
      state.files['src/demo.ts'] = 'export const demo = "OpenCode SDK 1.14.49";\n';
