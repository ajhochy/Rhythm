...
- AsyncStorage settings contain only the public server URL, username, and directory; the password is read from the device-only Keychain at foreground and native background execution time
...
### Via OpenCode 1.14.49 V2 SDK Client
...
- `projectPath`
- secret-free connection identity: `serverUrl`, `username`
- `requestedAt`

At foreground and native background execution time, the app obtains the matching password from the
device-only Keychain. Locked or unavailable Keychain access retains pending work for retry. Pending
records whose connection identity does not match the active credential are discarded rather than paired
