...
The client is implemented against the bundled `@opencode-ai/sdk` 1.14.49 contract and uses generated SDK types rather than permissive legacy endpoint shims. It is latest-only support: older server shapes and removed endpoint forms are not compatibility targets.
...
AsyncStorage persists only public connection identity (`serverUrl`, `username`, and `directory`) and
secret-free pending-notification metadata. The connection password is read from the device-only Keychain
when the foreground app or native background notification handler needs it.

Operational implication:

- unavailable or locked Keychain access keeps pending notification work for a later retry rather than
  using or deleting an empty credential
- a pending record is discarded if its public connection identity does not match the credential identity;
  credentials are never paired across connections

### Notification Tracking Storage

Pending notification sessions store only secret-free connection identity in AsyncStorage. Background checks
retrieve the current matching password from Keychain at execution time.
