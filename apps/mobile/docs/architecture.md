...
### Transport Clients (`lib/transport/`)

Two thin HTTP clients provide the authenticated transport layer for the Rhythm
Agent iOS features.  They sit below the provider layer and are consumed by
account, pairing, and tool-screen providers introduced in later tasks.

- `lib/transport/types.ts`
  Shared interfaces: `ApiErrorOptions`, `ApiErrorSource`, `RhythmCloudClientOptions`,
  `PairedMacClientOptions`, and the injected `FetchFn` type.
- `lib/transport/api-error.ts`
  `ApiError` (extends `Error`) with `source`, `status`, `code`, `message`, and
  `retryable` fields.  `normalizeApiError` actively scrubs auth tokens from
  server-supplied fields.  Three factory helpers cover network failures
  (`normalizeNetworkError`), token-provider failures (`normalizeProviderError`),
  and response-body read failures (`normalizeBodyReadError`).
- `lib/transport/request-helper.ts`
  `executeRequest` — the shared HTTP request implementation.  Handles token
  acquisition, conditional `Content-Type`, network errors, non-2xx
  normalization (with token scrubbing), and body-read failures.  Both clients
  delegate to this helper, differing only in their Authorization scheme.
- `lib/transport/rhythm-cloud-client.ts`
  `RhythmCloudClient` — authenticated with `Authorization: Bearer <token>`.
  Consumes the production Rhythm Cloud API.  Token stored under
  `rhythm.cloud.session` in SecureStore (managed by `RhythmAccountProvider`
  introduced in Task 3).
- `lib/transport/paired-mac-client.ts`
  `PairedMacClient` — authenticated with `Authorization: Device <token>`.
  Consumes the paired Mac's Tailscale mobile gateway.  Provides `sseUrl()`,
  `subscribe()` (alias for `sseUrl`), and `ptyUrl()` as URL-only helpers
  that do not open connections.  Token stored under `rhythm.paired.device`
  in SecureStore (managed by `PairedHostProvider` introduced in Task 9).

**Authorization scheme isolation:** `Bearer` vs `Device` are distinct HTTP auth
schemes, not just different header values.  A cloud token cannot be silently
accepted by the paired-Mac endpoint, and vice versa — server-side validation
rejects the wrong scheme with a clear 401.

**Error contract:** Every transport failure — token-provider rejection, network
failure, non-2xx status, body-read failure — is normalized to `ApiError` before
leaving `lib/transport/`.  Higher-level state machines (account, pairing) can
depend on `source` and `retryable` without parsing error messages.

### OpenCode Protocol Helpers

- `lib/opencode/client.ts`
  Builds the OpenCode 1.14.49 v2 SDK client, normalizes server URL, adds optional basic auth, and preserves configured URL path prefixes.
