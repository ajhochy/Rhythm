...
This is the client-side OpenCode contract implemented by the app. The code targets `@opencode-ai/sdk` 1.14.49 and imports generated request and response types directly. It is latest-only support: the app does not carry compatibility shims for older OpenCode endpoint shapes.
