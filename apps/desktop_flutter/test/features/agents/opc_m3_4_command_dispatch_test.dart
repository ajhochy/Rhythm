...
void main() {
  ...
  group('issue-697-c2: popover command selection sends session.command frame',
    ...
    testWidgets(
      ...
      agentsCtrl.setMessageForTest(ChatMessage(
        id: 'msg-init',
        sessionId: sessionId,
        role: 'assistant',
        createdAt: _kEpoch,
      ...
      expect(
          repo.sentFrames.any((f) => f['type'] == 'session.command'), isTrue);
      ...
      for (final f in inputFrames) {
        ...
        expect(data.startsWith('/help'), isFalse,
    ...
    testWidgets(
      ...
      expect(cmdFrames.isNotEmpty, isTrue,
      ...
      final inputTextFrames = repo.sentFrames
          ...
          .toList();
      expect(inputTextFrames.isEmpty, isTrue,
      ...
      final plainFrames =
          repo.sentFrames.where((f) => f['type'] == 'session.input').toList();
      expect(plainFrames.isNotEmpty, isTrue,
  ...
  testWidgets('issue-697-c3: command role message renders /name args row',
    ...
    expect(commandMessages.isNotEmpty, isTrue,
    ...
    expect(invocationText, contains('/help'),
        reason: 'Command invocation text must start with /commandName');
    expect(invocationText, contains('some-topic'),
  ...
  test(
    ...
    repo.injectWsMessage(const WsErrorMessage(
      id: 'test-session-c5',
    ...
    expect(systemMessages.isNotEmpty, isTrue,
