...
class _FakeAgentsRepository implements AgentsRepository {
  ...
  /// session-create failure (e.g. a raw server validation string).
...
void main() {
  ...
  // --------------------------------------------------------------------------
  // createSession error sanitization (#1154)
  // --------------------------------------------------------------------------
  ...
  group('createSession error sanitization (#1154)', () {
    ...
    test(
      ...
      expect(controller.error, equals('agent not configured'));
    });
  });
  ...
  group('WS messages update state', () {
    ...
    test(
      'SessionCreatedMessage does not duplicate an existing session',
      () async {
        fakeRepo.emit(
          SessionCreatedMessage(
    ...
    test(
      ...
      () async {
        ...
        fakeRepo.emit(
          const SessionClosedMessage(id: 'to-close', resumable: true),
        );
        await Future<void>.delayed(Duration.zero);
    ...
    test(
      ...
      () async {
        ...
        fakeRepo.emit(const SessionClosedMessage(id: 'temp', resumable: false));
        await Future<void>.delayed(Duration.zero);
    ...
    test('OutputMessage clears sessionFirstSeenAt (stuck tracking)', () async {
      ...
      // Mark as stuck.
      controller.sessionFirstSeenAt[sessionId] = DateTime.now().subtract(
        const Duration(seconds: 40),
      );
      expect(controller.sessionFirstSeenAt.containsKey(sessionId), isTrue);

      fakeRepo.emit(
        const OutputMessage(id: sessionId, data: 'hello world', replay: false),
      );
      await Future<void>.delayed(Duration.zero);
  ...
  group('connectivity stream transitions', () {
    ...
    test(
      'stream emitting false sets isWsDisconnected to true and notifies',
      () async {
    ...
    test(
      'redundant true event does not trigger extra notifyListeners',
      () async {
    ...
    test(
      'redundant false event does not trigger extra notifyListeners',
      () async {
        // Disconnect first.
        fakeRepo.emitConnectivity(false);
        await Future<void>.delayed(Duration.zero);
  ...
  group('dismissTrigger()', () {
    ...
    test('removes the matching pending trigger', () async {
      fakeRepo.emit(
        const TriggerFiredMessage(taskId: 'task-1', taskTitle: 'Task One'),
      );
      fakeRepo.emit(
        const TriggerFiredMessage(taskId: 'task-2', taskTitle: 'Task Two'),
      );
      await Future<void>.delayed(Duration.zero);
  ...
  group('stuck-session detection', () {
    ...
    test(
      'session with no output and >30s elapsed appears in stuckSessionIds',
      () async {
        ...
        controller.sessionFirstSeenAt['stuck-sess'] = DateTime.now().subtract(
    ...
    test('session with output is not considered stuck', () async {
      ...
      controller.sessionFirstSeenAt['active-sess'] = DateTime.now().subtract(
        const Duration(seconds: 31),
      );
      ...
      expect(
        controller.connectivity.stuckSessionIds,
        isNot(contains('active-sess')),
      );
    });

    test(
      'output message clears session from sessionFirstSeenAt immediately',
      () async {
        fakeRepo.emit(
          SessionCreatedMessage(
        ...
        fakeRepo.emit(
          const OutputMessage(id: 'out-sess', data: 'hello', replay: false),
        );
        await Future<void>.delayed(Duration.zero);
    ...
    test(
      'closed session is removed from stuckSessionIds on next tick',
      () async {
        fakeRepo.emit(
          SessionCreatedMessage(
        ...
        controller.sessionFirstSeenAt['closing-sess'] = DateTime.now().subtract(
          const Duration(seconds: 31),
        );
        controller.recomputeStuckForTest();
        expect(
          controller.connectivity.stuckSessionIds,
          contains('closing-sess'),
        );

        // Now close the session.
        fakeRepo.emit(
          const SessionClosedMessage(id: 'closing-sess', resumable: false),
        );
        await Future<void>.delayed(Duration.zero);

        // sessionFirstSeenAt entry should be gone.
        expect(
          controller.sessionFirstSeenAt.containsKey('closing-sess'),
          isFalse,
        );
    ...
    test('session <30s old is not yet stuck', () async {
      ...
      controller.sessionFirstSeenAt['young-sess'] = DateTime.now().subtract(
    ...
    test(
      ...
      () async {
        ...
        expect(
          controller.sessionFirstSeenAt.containsKey('list-starting'),
          isTrue,
        );
    ...
    test('notifyListeners fires when stuckSessionIds changes', () async {
      ...
      controller.sessionFirstSeenAt['notify-sess'] = DateTime.now().subtract(
        const Duration(seconds: 31),
      );
  ...
  group('reconnectSession()', () {
    test('when server not ready: calls retry() then load()', () async {
      final notReadyServerController = _FakeAgentServerController(
        ready: false,
        anyAgent: false,
      );
      final localController = AgentsController(
      ...
      fakeRepo.sessionsToReturn = [_makeSession('s1', AgentSessionStatus.idle)];
    ...
    test(
      'when server ready: sends session.subscribe and refreshes transcript',
      () async {
        final readyServerController = _FakeAgentServerController(
          ready: true,
          anyAgent: true,
        );
        final localController = AgentsController(
  ...
  group('closeSession()', () {
    test(
      ...
      () async {
        ...
        expect(
          localController.sessionFirstSeenAt.containsKey('stale-sess'),
          isFalse,
        );
      },
    );
