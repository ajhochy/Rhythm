...
List<AgentSessionMessage> _makeChildMessages() => [
  AgentSessionMessage.fromStructuredJson({
    ...
    'parts': [
      {'id': 'part-child-text-001', 'type': 'text', 'text': 'Fix the auth bug'},
    ],
    'tokens': null,
    'cost': null,
  }),
...
void main() {
  ...
  group('issue-699-c2: tapping a task chip shows child transcript view', () {
    testWidgets(
      ...
      (tester) async {
                  ...
                  return Column(
                    children: [
                      ...
                      TextButton.icon(
                        ...
                        label: Text(
                          '‹ ${agentsCtrl.activeChildParentName ?? ''}',
                        ),
                      ),
        ...
        expect(
          repo.fetchParentSessionCallCount,
          equals(initialFetchCount),
  ...
  group('issue-699-c3: breadcrumb back returns to parent without refetch', () {
    test(
      'closeChildSession clears active child and does not refetch parent',
      () async {
            ...
            Builder(
              builder: (ctx) {
                ...
                return ChildTranscriptView(
                  childSdkId: childId,
                  parentSessionName:
                      agentsCtrl.activeChildParentName ?? 'Parent',
                  onBack: () => agentsCtrl.closeChildSession(),
                );
              },
            ),
  ...
  group(
    'issue-699-c5: child sessions never appear in active/resumable lists',
    () {
      test(
  ...
  group('issue-699-c6: chip ToolState indicator updates in parent transcript', () {
    testWidgets(
      ...
      (tester) async {
        ...
        final runningPart = _makeTaskPart(
          messageId: messageId,
          toolStatus: 'running',
        );
        ...
        final completedPart = _makeTaskPart(
          messageId: messageId,
          toolStatus: 'completed',
        );
