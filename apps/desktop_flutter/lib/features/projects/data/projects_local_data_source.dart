...
class ProjectsLocalDataSource {
  ...
  Future<List<ProjectTemplate>> fetchAll() async {
    ...
    return list.map((j) => ProjectTemplate.fromJson(j as Map<String, dynamic>)).toList();
  }

  Future<ProjectTemplate> create(String name, {String? description, String? anchorType}) async {
  ...
  Future<ProjectTemplateStep> addStep(
    ...
    final response = await http.post(
      Uri.parse('${AppConstants.apiBaseUrl}/project-templates/$templateId/steps'),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({
        'title': title,
