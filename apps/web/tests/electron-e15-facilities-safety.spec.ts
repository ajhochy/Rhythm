...
async function intercept(page: Page) {
  ...
  await page.route('**/*', async (route) => {
    ...
    const headers = { 'access-control-allow-origin': 'http://127.0.0.1:4180', 'access-control-allow-headers': 'authorization,content-type', 'access-control-allow-methods': 'GET,DELETE,OPTIONS' };
    ...
    if (['http://127.0.0.1:4098', 'http://127.0.0.1:4097', 'https://api.vcrcapps.com'].includes(url.origin)) {
      ...
      if (request.method() === 'GET') {
        ...
        if (url.pathname === '/facilities/reservations') return reply([reservation]);
  ...
  return { deletes, allowRoomDelete: () => { failRoomDelete = false; }, delayPreview: () => { delayPreview = true; }, previewPending: () => releasePreview !== null, releasePreview: () => releasePreview?.() };
}
