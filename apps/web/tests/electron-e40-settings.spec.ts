...
test('E40: settings gateway uses authenticated workspace and preference routes', async () => {
  ...
  const gateway = createLiveSettingsGateway('https://settings.invalid', 'settings-token', async (input, init) => {
    const url = new URL(String(input)); seen.push({ path: url.pathname, method: init?.method ?? 'GET', body: init?.body ? JSON.parse(String(init.body)) : null });
  ...
  expect(seen).toEqual(expect.arrayContaining([
    { path: '/workspaces/me', method: 'GET', body: null }, { path: '/workspaces/me/members', method: 'GET', body: null },
    { path: '/workspaces/me/members/2/role', method: 'PATCH', body: { role: 'admin' } }, { path: '/users/2', method: 'PATCH', body: { isFacilitiesManager: true } },
    { path: '/users/me/preferences', method: 'PATCH', body: { emailNotificationsEnabled: false } },
  ]));
});
