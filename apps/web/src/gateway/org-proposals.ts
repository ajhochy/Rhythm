...
export type OrgProposal = { id: string; title: string; kind: string; risk: string; status: string; outcomeStatus: string; rationale: string | null; createdAt: string | null; updatedAt: string | null; changeJson: string | null; toolSafety?: ToolSafetyProjection; experimentSummary?: { safeExperimentSummary?: string | null } | null; };
...
function toolSafety(value: unknown): ToolSafetyProjection {
  ...
  if (raw.state !== 'ready' || !['safe', 'conditional', 'unsafe'].includes(String(raw.verdict)) || !tool || !text(tool.name) || !text(tool.packageSource) || !paths || paths.some((entry) => !entry || !text(entry.label) || nonnegative(entry.count) === null) || !hosts || hosts.some((entry) => !entry || !text(entry.host) || nonnegative(entry.count) === null) || nonnegative(raw.workspaceWriteCount) === null || nonnegative(raw.credentialAccessAttemptsCount) === null || nonnegative(raw.scenarioAttemptsCount) === null || nonnegative(raw.sandboxDurationMs) === null || (raw.reason !== null && raw.reason !== undefined && !text(raw.reason))) return { state: 'malformed', verdict: 'unknown' };
