...
export function Profiles() {
  ...
  useEffect(() => { setDraft(structuredClone(selected)); setRenaming(false); setSaveError(''); }, [selected.id, selectedId, selected.updatedAt]);
  ...
  const set = <K extends keyof IdentityProfile>(key: K, value: IdentityProfile[K]) => setDraft((current) => ({ ...current, [key]: value }));
  const save = async (patch: Partial<IdentityProfile> = draft) => {
    if (saving) return;
    setSaving(true); setSaveError('');
    try { setSelectedId(await updateProfile(selected.id, patch)); setRenaming(false); notify('Profile saved'); }
  ...
  return (
    <section className="profiles-workspace" aria-label="Agent profiles" aria-describedby={fixtureState === 'read-only' ? 'profiles-readonly' : undefined} data-od-id="profiles-workspace" data-testid="profiles-workspace">
      ...
      <aside className="profile-rail" aria-label="Profile list">
        ...
        <div className="profile-list">{visible.map((profile) => <button className={`profile-row ${selectedId === profile.id ? 'selected' : ''}`} type="button" key={profile.id} onClick={() => setSelectedId(profile.id)} data-testid={`profile-${profile.id}`}><ProfileAvatar profile={profile} /><span><strong>{profile.label}</strong><small>{profile.modelProvider ?? 'Configured'} · {profile.modelId ?? 'Configured model'}</small></span>{profile.isDefault && <em>Default</em>}{!profile.enabled && <em>Disabled</em>}</button>)}</div>
      </aside>
      {!selected.id && <p role="status">No profiles configured. Create a profile to begin.</p>}
      <fieldset className="profile-editor profile-editor-fieldset" aria-labelledby="profile-editor-title" disabled={fixtureState === 'read-only' || !selected.id || saving}>
        <header className="profile-editor-header"><div className="editor-title"><ProfileAvatar profile={draft} size="large" /><div>{renaming ? <form onSubmit={(event) => { event.preventDefault(); void save({ label: draft.label }); }}><label className="sr-only" htmlFor="inline-profile-name">Profile name</label><input id="inline-profile-name" autoFocus required value={draft.label} onChange={(event) => set('label', event.target.value)} data-testid="profile-inline-name" /><button className="icon-button small" type="submit" aria-label="Confirm rename" data-testid="profile-inline-confirm"><Icon name="check" size={14} /></button></form> : <div className="profile-title-line"><h2 id="profile-editor-title">{selected.label}</h2><button className="icon-button small" type="button" onClick={() => setRenaming(true)} aria-label="Rename profile inline" data-testid="profile-rename"><Icon name="rename" size={14} /></button></div>}<p>{selected.isDefault ? 'Default agent profile' : 'Workspace agent profile'} · {live ? selected.updatedAt : 'updated Aug 12'}</p></div></div><div className="editor-header-actions"><button className="secondary-button" type="button" onClick={() => { const id = duplicateProfile(selected.id); setSelectedId(id); }} data-testid="profile-duplicate"><Icon name="copy" size={14} />Duplicate</button><button className="secondary-button" type="button" onClick={() => setDefaultProfile(selected.id)} disabled={selected.isDefault || live && (!selected.enabled || !selected.selectable || selected.id.startsWith('profile-created-'))} title={selected.isDefault ? 'Already the default profile' : undefined} data-testid="profile-default"><Icon name="check" size={14} />{selected.isDefault ? 'Default' : 'Set default'}</button><button className="text-danger-button" type="button" onClick={() => selected.isDefault ? notify('Choose another default before deleting this profile') : setDeleteOpen(true)} data-testid="profile-delete"><Icon name="delete" size={14} />Delete</button></div></header>
        {live && <p>Default profile is local to this account and resets on reload; not a server preference.</p>}
        {saveError && <p role="alert">{saveError}</p>}
        {live && catalogError && <p role="alert">{catalogError}</p>}
        <form className="profile-form" onSubmit={(event) => { event.preventDefault(); void save(); }}>
          <section className="editor-section"><header><div><h3>Identity</h3><p>How this profile appears and guides each session.</p></div></header><div className="form-grid"><label className="field">{!looksLikeAssetPath(draft.icon) && draft.icon.length <= 3 ? 'Icon label' : 'Icon value'}<input value={draft.icon} maxLength={!looksLikeAssetPath(draft.icon) && draft.icon.length <= 3 ? 3 : undefined} onChange={(event) => set('icon', !looksLikeAssetPath(draft.icon) && draft.icon.length <= 3 ? event.target.value.toUpperCase() : event.target.value)} data-testid="profile-icon" /></label><label className="field">Profile label<input value={draft.label} onChange={(event) => set('label', event.target.value)} data-testid="profile-label" /></label><label className="field span-2">System prompt<textarea rows={5} value={draft.systemPrompt} onChange={(event) => set('systemPrompt', event.target.value)} data-testid="profile-system-prompt" /></label></div></section>
          ...
          <section className="editor-section"><header><div><h3>Provider &amp; model</h3><p>Defaults used when a session begins with this profile.</p></div></header><div className="form-grid">
          ...
          <section className="editor-section"><header><div><h3>Core permission rules</h3><p>Ask, allow, or deny each capability by default.</p></div></header>{live ? <label className="field">Canonical permission policy (JSON)<textarea rows={6} value={draft.corePermissionsJson ?? ''} onChange={event => set('corePermissionsJson', event.target.value || null)} data-testid="profile-permissions" /><span>Blank inherits engine policy. Supports action strings and per-tool pattern objects; saved without flattening.</span></label> : <div className="permission-table" role="table" aria-label="Core permission rules"><div role="row" className="permission-head"><span role="columnheader">Capability</span><span role="columnheader">Ask</span><span role="columnheader">Allow</span><span role="columnheader">Deny</span></div>{Object.entries(draft.permissionRules).map(([rule, value]) => <div role="row" key={rule}><strong role="cell">{rule}</strong>{(['ask', 'allow', 'deny'] as const).map((choice) => <label role="cell" key={choice}><input type="radio" name={`permission-${rule}`} checked={value === choice} onChange={() => setDraft((current) => ({ ...current, permissionRules: { ...current.permissionRules, [rule]: choice } }))} aria-label={`${rule}: ${choice}`} data-testid={`permission-${rule}-${choice}`} /></label>)}</div>)}</div>}</section>
