  ...
  return (
    <section className="profiles-workspace" aria-label="Agent profiles" aria-describedby={fixtureState === 'read-only' ? 'profiles-readonly' : undefined} data-od-id="profiles-workspace" data-testid="profiles-workspace">
      ...
      <fieldset className="profile-editor profile-editor-fieldset" aria-labelledby="profile-editor-title" disabled={fixtureState === 'read-only'}>
        ...
        <form className="profile-form" onSubmit={(event) => { event.preventDefault(); void updateProfile(selected.id, draft).then(setSelectedId).then(() => notify('Profile saved')).catch(() => notify('Profile could not be saved')); }}>
          ...
          <section className="editor-section"><header><div><h3>Provider &amp; model</h3><p>Defaults used when a session begins with this profile.</p></div></header><div className="form-grid"><label className="field">Provider<select value={draft.modelProvider ?? ''} onChange={(event) => set('modelProvider', event.target.value || null)} data-testid="profile-provider"><option value="">No preference</option><option value="openai">OpenAI</option><option value="anthropic">Anthropic</option><option value="local">Local provider</option></select></label><label className="field">Model<select value={draft.modelId ?? ''} onChange={(event) => set('modelId', event.target.value || null)} data-testid="profile-model"><option value="">No preference</option><option value="gpt-5.6">gpt-5.6</option><option value="gpt-5.6-codex">gpt-5.6-codex</option><option value="claude-sonnet-4">claude-sonnet-4</option><option value="claude-sonnet-4-6">claude-sonnet-4-6</option></select></label><label className="field span-2">{live ? 'Default Anthropic account ID' : 'Default account'}{live ? <><input value={draft.defaultAnthropicAccountId ?? ''} onChange={(event) => set('defaultAnthropicAccountId', event.target.value.trim() || null)} data-testid="profile-account" /><span>Use an existing account ID, or leave blank for no default.</span></> : <select value={draft.defaultAccount} onChange={(event) => set('defaultAccount', event.target.value)} data-testid="profile-account"><option>Rhythm workspace</option><option>Research account</option><option>Local account</option></select>}</label></div></section>
