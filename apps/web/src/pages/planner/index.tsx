...
function TaskCard({ task, readonly, selected, selectionMode, onInspect, onComplete, onSelect, onDragStart }: {
  task: PlannerTask;
  readonly: boolean;
  selected: boolean;
  ...
  return (
    <article className={`planner-task ${task.source === 'project-step' ? 'project-step' : ''} ${selected ? 'selected' : ''}`} data-status={task.status}>
      <button className="task-complete" type="button" disabled={readonly} aria-describedby={readonly ? 'planner-readonly-reason' : undefined} aria-label={`${task.status === 'done' ? 'Reopen' : 'Complete'} ${task.title}`} onClick={() => onComplete(task)} data-testid={`planner-complete-${task.id}`}><span aria-hidden="true">✓</span></button>
      <button
        className="task-main"
        type="button"
        draggable={!readonly && task.status !== 'done'}
      ...
      {selectionMode && <button className="task-select" type="button" disabled={readonly || task.status === 'done'} aria-label={`Select ${task.title}`} aria-pressed={selected} onClick={() => onSelect(task)} data-testid={`planner-task-select-${task.id}`}>{selected ? 'Selected' : 'Select'}</button>}
    </article>
  );
}
...
function usePlannerLayout() {
  ...
  useEffect(() => {
    ...
    if (!node) return;
    const observer = new ResizeObserver(([entry]) => setNarrow(entry.contentRect.width < 700));
...
function PlannerToolbar({ layout, week, backlog, all, onFilter, onWeek, onToday, onCreate, disabled, onSelection }: {
  layout: ReturnType<typeof usePlannerLayout>; week: string; backlog: number; all: boolean;
  onFilter(all: boolean): void; onWeek(week: string): void; onToday(): void; onCreate(): void;
  disabled: boolean; onSelection(): void;
}) {
  return <header className="planner-toolbar">
    <h1 id="planner-title">Planner</h1>
    <nav className="week-controls" aria-label="Week navigation">
      <button className="secondary-button" type="button" aria-label="Previous week" onClick={() => onWeek(adjacentWeek(week, -1))} data-testid="planner-prev-week">←</button>
      <span data-testid="planner-week-label">Week of {weekFormatter.format(isoWeekMonday(week))}</span>
      <button className="secondary-button" type="button" aria-label="Next week" onClick={() => onWeek(adjacentWeek(week, 1))} data-testid="planner-next-week">→</button>
      <button className="secondary-button" type="button" onClick={onToday} data-testid="planner-today">Today</button>
    </nav>
    <div className="planner-header-actions">
      <button ref={layout.backlogTrigger} className="secondary-button" type="button" aria-expanded={layout.backlogOpen} aria-controls={layout.backlogOpen ? 'planner-backlog-panel' : undefined} onClick={() => { if (layout.root.current?.querySelector('.planner-board')) layout.setBacklogOpen(!layout.backlogOpen); }}>Backlog {backlog}</button>
      <div className="filter-control" role="group" aria-label="Task visibility"><button type="button" aria-pressed={!all} onClick={() => onFilter(false)} data-testid="planner-filter-open">Open</button><button type="button" aria-pressed={all} onClick={() => onFilter(true)} data-testid="planner-filter-all">All</button></div>
      <button className="secondary-button" type="button" aria-pressed={layout.selectionMode} disabled={disabled} onClick={onSelection}>Select tasks</button>
      <HeaderTaskAction onClick={onCreate} disabled={disabled} testId="planner-header-add-task" />
      <div className="planner-view" role="group" aria-label="Planner presentation"><button type="button" aria-pressed={!layout.agenda} onClick={() => layout.setView('week')}>Week</button><button type="button" aria-pressed={layout.agenda} onClick={() => layout.setView('agenda')}>Agenda</button></div>
    </div>
  </header>;
}
...
function FixturePlannerPage({ route }: { route: string }) {
  ...
  const moveTask = (taskId: string, date: string) => {
    ...
    setTasks((current) => current.map((item) => item.id === taskId ? { ...item, ...(item.source === 'project-step' ? { dueDate: date } : { scheduledDate: date }), scheduledOrder: 900 } : item));
    if (task.source === 'project-step') appendReceipt(`PATCH /project-instances/steps/${task.id} {dueDate:${date}} → 200`);
    else appendReceipt(`PATCH /weekly-plan/tasks/${task.id} {scheduledDate:${date}} → 200`);
  ...
  return (
    <section ref={layout.root} className={`page-shell pg-planner ${layout.agenda ? 'planner-agenda' : 'planner-week'}`} data-testid="page-planner" aria-labelledby="planner-title">
      <PlannerToolbar layout={layout} week={week} backlog={backlog.length} all={filter === 'all'} onFilter={(all) => setFilter(all ? 'all' : 'open')} onWeek={changeWeek} onToday={() => { changeWeek(PLANNER_CURRENT_WEEK); layout.focusToday(); }} onCreate={() => setInspector({ kind: 'create' })} disabled={!contentVisible || readonly} onSelection={() => { layout.setSelectionMode(!layout.selectionMode); setSelectedIds([]); }} />
      ...
      <div className="planner-scroll">
        ...
        {contentVisible && <>
          ...
          {selectedIds.length > 0 && <aside className="selection-bar" aria-label="Selected tasks"><strong data-testid="planner-selection-count">{selectedIds.length} selected</strong><button className="primary-button" type="button" disabled={readonly} onClick={bulkComplete} data-testid="planner-bulk-complete">Complete selected</button><button className="text-button" type="button" disabled={readonly} onClick={() => setSelectedIds([])} data-testid="planner-clear-selection">Clear</button></aside>}

          <section className="planner-board" aria-label="Weekly plan" tabIndex={0}>
            {layout.backlogOpen && <aside className="backlog-lane" id="planner-backlog-panel" data-testid="planner-backlog" onKeyDown={(event) => { if (event.key === 'Escape') { event.stopPropagation(); layout.closeBacklog(); } }}>
              ...
              <button className="text-button" type="button" onClick={layout.closeBacklog}>Close backlog</button>
      ...
      <details className="planner-diagnostics"><summary>Diagnostics</summary><div className="planner-summary"><span data-testid="planner-summary-open">{scheduledOpen}</span><span data-testid="planner-summary-done">{doneCount}</span><span data-testid="planner-summary-unscheduled">{backlog.length}</span></div><aside className="page-trace" aria-label="Planner endpoint receipts" data-testid="page-trace"><span>Wire receipts</span><ol>{receipts.map((receipt, index) => <li key={`${index}-${receipt}`}>{receipt}</li>)}</ol></aside></details>
      ...
      <FocusDialog open={Boolean(currentTask)} onClose={closeInspector} title={readonly ? 'Task details' : currentTask?.source === 'project-step' ? 'Edit project step' : 'Edit task'} description={currentTask?.source === 'project-step' ? `Source-owned by ${currentTask.projectName}. Only notes and due date persist from Planner.` : readonly ? 'Inspection remains available with read-only Planner permission.' : 'Planner persists notes and date fields for existing tasks.'} testId="planner-inspector">
        {currentTask && (readonly ? <article className="readonly-details"><span className="eyebrow">Read only</span><h3>{currentTask.title}</h3><p>{currentTask.notes || 'No notes'}</p><dl><div><dt>Scheduled</dt><dd>{currentTask.scheduledDate ?? 'Unscheduled'}</dd></div><div><dt>Due</dt><dd>{currentTask.dueDate ?? 'No due date'}</dd></div></dl></article> : <>
          <form key={currentTask.id} id="planner-edit-form" className="inspector-form task-editor-form" onSubmit={saveTask} onChange={(event) => { if (['notes', 'scheduledDate', 'dueDate'].includes((event.target as HTMLInputElement).name)) setDirty(true); }}>
            <div className="planner-status-action"><button className="primary-button" type="button" onClick={() => toggleComplete(currentTask)} data-testid="planner-detail-complete">{currentTask.status === 'done' ? 'Reopen task' : 'Complete task'}</button></div>
            {confirmClose && <div role="alert">Unsaved changes<button className="secondary-button" type="submit">Save</button><button className="secondary-button" type="button" onClick={() => { setDirty(false); setConfirmClose(false); setInspector(null); }}>Discard</button><button className="secondary-button" type="button" onClick={() => setConfirmClose(false)}>Keep editing</button></div>}
            ...
            <details className="planner-move"><summary>Move task</summary><label>Move to date<input type="date" value={moveDate} onChange={event => setMoveDate(event.target.value)} /></label><button className="secondary-button" type="button" disabled={!moveDate || currentTask.status === 'done'} onClick={() => moveTask(currentTask.id, moveDate)}>Move</button>{currentTask.source === 'task' && [-1, 1].map(direction => {
              const siblings = tasks.filter(task => task.source === 'task' && task.scheduledDate === currentTask.scheduledDate && task.status === 'open').sort((a, b) => a.scheduledOrder - b.scheduledOrder);
              const index = siblings.findIndex(task => task.id === currentTask.id);
              const neighbor = siblings[index + direction];
              return <button key={direction} className="secondary-button" type="button" disabled={index < 0 || !neighbor} title={!neighbor ? 'No editable task in that direction' : undefined} aria-label={`Move ${currentTask.title} ${direction < 0 ? 'earlier' : 'later'}`} onClick={() => {
                if (!neighbor) return;
                setTasks(current => current.map(task => task.id === currentTask.id ? { ...task, scheduledOrder: neighbor.scheduledOrder + direction } : task));
                appendReceipt(`PATCH /tasks/${currentTask.id} {scheduledOrder} → 200`);
              }}>Move {direction < 0 ? 'earlier' : 'later'}</button>;
            })}</details>
...
function LiveTaskCard({ task, selected, selectionMode, pending, onInspect, onComplete, onSelect, onDragStart }: {
  task: Task;
  selected: boolean;
  selectionMode: boolean;
  pending: boolean;
  ...
  return (
    <article className={`planner-task ${isProjectStep(task) ? 'project-step' : ''} ${selected ? 'selected' : ''}`} data-status={task.status} data-source-type={task.sourceType ?? undefined} data-source-id={task.id}>
      {!readonly && <button className="task-complete" type="button" disabled={pending} aria-label={`${task.status === 'done' ? 'Reopen' : 'Complete'} ${task.title}`} onClick={() => onComplete(task)} data-testid={`planner-complete-${task.id}`}><span aria-hidden="true">✓</span></button>}
      <button className="task-main" type="button" draggable={!readonly && !pending && task.status !== 'done' && task.status !== 'deferred'} aria-label={`Inspect ${task.title}`} onDragStart={(event) => onDragStart(event, task)} onClick={() => onInspect(task)} data-testid={`planner-task-${task.id}`} data-task-title="true">
        <strong>{task.title}</strong>
        <span className="task-source">{readonly ? isCalendarShadow(task) ? 'Calendar · read only' : 'Read only · update in source' : sourceLabel}</span>
        {eventTime && <small data-testid={`planner-task-time-${task.id}`}>{eventTime}</small>}
        {task.dueDate && task.dueDate !== (isProjectStep(task) ? task.dueDate : task.scheduledDate) ? <small data-testid={`planner-task-due-${task.id}`}>Due {task.dueDate}</small> : null}
        {task.status !== 'open' && task.status !== 'done' && <small>{task.status.replaceAll('_', ' ')}</small>}
      </button>
      {!readonly && selectionMode && <button className="task-select" type="button" disabled={pending || task.status === 'done' || task.status === 'deferred'} aria-label={`Select ${task.title}`} aria-pressed={selected} onClick={() => onSelect(task)} data-testid={`planner-task-select-${task.id}`}>{selected ? 'Selected' : 'Select'}</button>}
    </article>
  );
}
...
function LivePlannerPage({ route }: { route: string }) {
  ...
  const [confirmClose, setConfirmClose] = useState(false);
  const [moveDate, setMoveDate] = useState('');
  const retainedTask = useRef<Task | null>(null);
  const statusPending = useRef(false);
  const [retryStatusTask, setRetryStatusTask] = useState<Task | null>(null);
  ...
  const { members: memberOptions, status: memberStatus, currentUserId } = useWorkspaceMembers();
  ...
  const recordError = (method: string, path: string, error: unknown, blocking = !plan): string => {
    ...
    if (blocking) setSurfaceState(status === 401 || status === 403 ? 'forbidden' : status === 404 ? 'unavailable' : 'server-error');
    return message;
  };
  ...
  const load = async (targetWeek: string, { blocking = true }: { blocking?: boolean } = {}) => {
    ...
    try {
      ...
      if (blocking) {
        setErrorMessage(recordError('GET', `/weekly-plan?week=${targetWeek}`, error, true));
      } else {
        const status = error instanceof PlannerGatewayError ? error.status : 0;
        const message = error instanceof PlannerGatewayError ? error.message : 'Planner service unavailable';
  ...
  const currentTask = inspectorTaskId ? allTasks.find((task) => task.id === inspectorTaskId) ?? (retainedTask.current?.id === inspectorTaskId ? retainedTask.current : null) : null;
  if (currentTask) retainedTask.current = currentTask;
  ...
  const toggleComplete = async (task: Task) => {
    if (statusPending.current || mutationPending || isCalendarShadow(task) || task.sourceType === 'prod_mirror') return;
    statusPending.current = true;
    setErrorMessage(null);
    setRetryStatusTask(null);
    ...
    try {
      ...
      if (retainedTask.current?.id === task.id) retainedTask.current = { ...retainedTask.current, status: nextStatus };
      setSelectedIds((current) => current.filter((id) => id !== task.id));
      notify(nextStatus === 'done' ? 'Task marked complete.' : `${task.title} reopened`);
      await load(week, { blocking: false });
    } catch (error) { setRetryStatusTask(task); setErrorMessage(recordError('PATCH', `${isProjectStep(task) ? '/project-instances/steps' : '/tasks'}/${task.id}`, error)); } finally { setMutationPending(false); statusPending.current = false; }
  };
  ...
  const moveTask = async (taskId: string, date: string) => {
    ...
    if (!task || mutationPending || isCalendarShadow(task) || task.sourceType === 'prod_mirror' || task.status === 'done' || task.status === 'deferred') return;
    setMutationPending(true);
    try {
      if (isProjectStep(task)) {
  ...
  const completeSelected = async () => {
    ...
    try {
      for (const id of selectedIds) {
        ...
        if (!task || isCalendarShadow(task) || task.sourceType === 'prod_mirror') continue;
        try {
          ...
          updateVisibleTask(id, 'done');
          setSelectedIds(current => current.filter(selected => selected !== id));
        } catch { failed.push(task.title); }
      }
      await load(week, { blocking: false });
      if (failed.length) setErrorMessage(`Could not complete: ${failed.join(', ')}. Failed tasks remain selected.`);
      else { appendReceipt(`PATCH ${selectedIds.length} selected planner records {status:"done"} → 200`); notify('Selected tasks completed'); }
  ...
  const closeInspector = () => { if (dirty) { setConfirmClose(true); return; } setInspectorTaskId(null); setCollaborators([]); };
  ...
  const saveTask = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      notify('Planner-supported details saved');
      setDirty(false); setConfirmClose(false); setInspectorTaskId(null); setCollaborators([]);
      await load(week);
  ...
  const visibleBacklog = showCompleted ? backlog : backlog.filter((task) => task.status !== 'done' && task.status !== 'deferred');
  ...
  return (
    <section ref={layout.root} className={`page-shell pg-planner ${layout.agenda ? 'planner-agenda' : 'planner-week'}`} data-testid="page-planner" aria-labelledby="planner-title">
      <PlannerToolbar layout={layout} week={week} backlog={visibleBacklog.length} all={showCompleted} onFilter={setShowCompleted} onWeek={changeWeek} onToday={() => { changeWeek(isoWeekLabelForDate(new Date())); layout.focusToday(); }} onCreate={() => { setCreateScheduledDate(undefined); setCreateOpen(true); }} disabled={(!contentVisible && surfaceState !== 'empty') || mutationPending} onSelection={() => { layout.setSelectionMode(!layout.selectionMode); setSelectedIds([]); }} />
      ...
      <div className="planner-scroll">
        ...
        {contentVisible && plan && <>
          {errorMessage && <div className="readonly-banner" role="alert" data-testid="planner-mutation-error">{errorMessage}<button className="secondary-button" type="button" disabled={mutationPending} onClick={() => retryStatusTask ? void toggleComplete(retryStatusTask) : void load(week, { blocking: false })}>Retry</button></div>}
      ...
      <FocusDialog open={Boolean(currentTask)} onClose={closeInspector} title={currentTask && isProjectStep(currentTask) ? 'Edit project step' : 'Edit task'} description={currentTask && isProjectStep(currentTask) ? `Source-owned by ${currentTask.sourceName ?? 'its project'}. Only notes and due date persist from Planner.` : 'Planner persists notes and date fields for existing tasks.'} testId="planner-inspector">
        {currentTask && (isCalendarShadow(currentTask) || currentTask.sourceType === 'prod_mirror' ? <article className="readonly-details"><span className="eyebrow">Read only · update in source</span><h3>{currentTask.title}</h3><p>{currentTask.notes || 'No notes'}</p></article> : <form key={currentTask.id} className="inspector-form task-editor-form" onSubmit={(event) => void saveTask(event)} onChange={(event) => { if (['notes', 'scheduledDate', 'dueDate'].includes((event.target as HTMLInputElement).name)) setDirty(true); }}>
          <div className="planner-status-action"><button className="primary-button" type="button" disabled={mutationPending} aria-busy={mutationPending} onClick={() => void toggleComplete(currentTask)} data-testid="planner-detail-complete">{currentTask.status === 'done' ? 'Reopen task' : 'Complete task'}</button>{errorMessage && <div role="alert">{errorMessage}<button className="secondary-button" type="button" disabled={mutationPending} onClick={() => retryStatusTask ? void toggleComplete(retryStatusTask) : void load(week, { blocking: false })}>Retry</button></div>}</div>
          {confirmClose && <div role="alert">Unsaved changes<button className="secondary-button" type="submit" disabled={mutationPending}>Save</button><button className="secondary-button" type="button" onClick={() => { setDirty(false); setConfirmClose(false); setInspectorTaskId(null); }}>Discard</button><button className="secondary-button" type="button" onClick={() => setConfirmClose(false)}>Keep editing</button></div>}
          <p className="inspector-record-title">{currentTask.title}</p>
          <label className="task-editor-field">Task notes<textarea name="notes" rows={4} defaultValue={currentTask.notes ?? ''} data-autofocus data-testid="planner-edit-notes" /></label>
          <div className="field-pair task-editor-pair">{!isProjectStep(currentTask) && <label className="task-editor-field">Scheduled date<input name="scheduledDate" type="date" defaultValue={currentTask.scheduledDate ?? ''} data-testid="planner-edit-scheduled-date" /></label>}<label className="task-editor-field">Due date<input name="dueDate" type="date" defaultValue={currentTask.dueDate ?? ''} data-testid="planner-edit-due-date" /></label></div>
          <details className="planner-move"><summary>Move task</summary><label>Move to date<input type="date" value={moveDate} onChange={event => setMoveDate(event.target.value)} /></label><button className="secondary-button" type="button" disabled={!moveDate || mutationPending || currentTask.status === 'done'} onClick={() => void moveTask(currentTask.id, moveDate)}>Move</button>{!isProjectStep(currentTask) && [-1, 1].map(direction => {
            const siblings = (plan?.days.find(day => day.tasks.some(task => task.id === currentTask.id))?.tasks ?? []).filter(task => !isCalendarShadow(task) && !isProjectStep(task) && task.sourceType !== 'prod_mirror' && task.status !== 'done' && task.status !== 'deferred');
            const index = siblings.findIndex(task => task.id === currentTask.id);
            const unavailable = index < 0 || !siblings[index + direction] || currentTask.status === 'done';
            return <button key={direction} className="secondary-button" type="button" disabled={mutationPending || unavailable} title={unavailable ? 'No editable task in that direction' : undefined} aria-label={`Move ${currentTask.title} ${direction < 0 ? 'earlier' : 'later'}`} onClick={() => void moveWithinDay(siblings, currentTask, direction as -1 | 1)}>Move {direction < 0 ? 'earlier' : 'later'}</button>;
          })}</details>
          {!isProjectStep(currentTask) && <fieldset className="collaborators task-editor-section" disabled={mutationPending || currentTask.isShared === true || (currentUserId !== null && currentTask.ownerId !== currentUserId)}><legend>People</legend>{(currentTask.isShared === true || (currentUserId !== null && currentTask.ownerId !== currentUserId)) && <p>Only the task owner can add or remove collaborators. You can still complete and edit this shared task.</p>}
