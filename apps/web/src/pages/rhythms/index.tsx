...
function RuleForm({ mode, initial, members, disabled = false, describedBy, onCancel, onSave }: { mode: 'create' | 'edit'; initial: RuleDraft; members: Array<{ id: string; name: string }>; disabled?: boolean; describedBy?: string; onCancel(): void; onSave(draft: RuleDraft): void }) {
  ...
  return <form className="rhythm-rule-form" onSubmit={submit}>
    ...
    <section className="rhythm-step-section" aria-labelledby={`${prefix}-steps-heading`}>
      ...
      {draft.steps.map((step, index) => { const stepTitleId = `rhythm-step-title-${index}`; const stepAssigneeId = `rhythm-step-assignee-${index}`; return <fieldset className="rhythm-step" key={step.id}><legend>Step {index + 1}</legend><div className="rhythm-step-grid"><label className="field rhythm-step-title" htmlFor={stepTitleId}>Task title<input id={stepTitleId} value={step.title} onChange={(event) => patchStep(index, { title: event.target.value })} data-testid={stepTitleId} /></label><label className="field" htmlFor={stepAssigneeId}>Assignee<select id={stepAssigneeId} value={step.assigneeId} onChange={(event) => patchStep(index, { assigneeId: event.target.value })} data-testid={stepAssigneeId}><option value="">None</option>{members.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label><ScheduleFields prefix="rhythm-step" suffix={`-${index}`} frequency={draft.frequency} dayOfWeek={step.dayOfWeek} dayOfMonth={step.dayOfMonth} month={step.month} onChange={(patch) => patchStep(index, patch)} /><button className="text-danger-button" type="button" onClick={() => setDraft((current) => ({ ...current, steps: current.steps.filter((_, stepIndex) => stepIndex !== index) }))} data-testid={`rhythm-remove-step-${index}`}>Remove step</button></div></fieldset>; })}
...
export function RhythmsPage({ route }: { route: string }) {
  ...
  const { members: memberOptions, status: memberStatus, currentUserId: liveUserId } = useWorkspaceMembers(rhythmWorkspaceMembers.map((person) => ({ ...person, userId: Number(person.id), email: '' })));
  ...
  const isOwnerOf = (ownerId: string) => live ? ownerId === String(liveUserId) : ownerId === currentRhythmUserId;
  ...
  const addCollaborator = async (personId: string) => {
    ...
    const person = memberOptions.find((member) => member.id === personId);
    if (!person) return;
    if (live) {
      if (!rhythmsGateway) return;
      setMutationPending(true);
  ...
  const candidates = useMemo(() => selected ? memberOptions.filter((person) => person.id !== selected.ownerId && !selected.collaborators.some((collaborator) => collaborator.id === person.id)) : [], [selected, memberOptions]);
  ...
  return <section className="page-shell pg-rhythms" data-testid="page-rhythms" aria-labelledby="rhythms-title" {...(selectedId ? { 'data-selected-stable-id': selectedId } : {})}>
    ...
    <main className="rhythms-main" aria-busy={surfaceState === 'loading'}>
      ...
      {statePanel}
      {showsWorkspace && <div className="rhythms-layout" style={{ '--rhythms-detail-width': `${detailWidth}px` } as React.CSSProperties}>
        ...
        <Splitter orientation="vertical" storageKey="layout.rhythms.detail" min={320} max={560} defaultSize={360} onResize={setDetailWidth} ariaLabel="Resize rhythm details" resizeEdge="end" testId="rhythms-detail-resizer" />
    ...
    {selected && <InspectorPortal selector="[data-testid='rhythm-detail']"><section className="rhythm-direct-editor" aria-label="Edit selected rhythm" data-testid="rhythm-direct-editor"><RuleForm key={selected.id} mode="edit" initial={cloneDraft(selected)} members={memberOptions} disabled={isReadonly || surfaceState === 'forbidden' || !isOwnerOf(selected.ownerId)} describedBy={!isOwnerOf(selected.ownerId) || surfaceState === 'forbidden' ? ownerOnlyReasonId : readonlyReasonId} onCancel={closeSelection} onSave={saveRule} /></section></InspectorPortal>}
    ...
    <FocusDialog open={createOpen} onClose={() => setCreateOpen(false)} title="New Recurring Rule" description="Create a recurring rule with optional workflow steps." testId="rhythm-create-dialog" wide>{memberStatus === 'error' && <p role="alert">Workspace members could not be loaded.</p>}<RuleForm mode="create" initial={blankDraft()} members={memberOptions} onCancel={() => setCreateOpen(false)} onSave={createRule} /></FocusDialog>
