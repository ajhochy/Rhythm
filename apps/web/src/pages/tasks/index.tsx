...
export function TasksPage({ route }: { route: string }) {
  ...
  const { members: memberOptions, status: memberStatus, currentUserId: liveUserId } = useWorkspaceMembers(workspaceMembers.map((person) => ({ ...person, userId: Number(person.id), email: '' })));
  ...
  const createTask = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const collaborator = memberOptions.find((person) => person.id === collaboratorId);
    if (gateway.mode === 'live') {
      setMutationPending(true);
      try {
        ...
        if (collaborator) {
          await gateway.domains.tasks!.addCollaborator(created.id, collaborator.userId);
          appendReceipt(`POST /tasks/${created.id}/collaborators {userId:${collaborator.userId}} → 200`);
        }
      ...
      if (event.key === 'Escape' && !collaboratorPickerOpen) { event.preventDefault(); event.stopPropagation(); closeInspector(); }
            ...
            <section className="task-detail" aria-labelledby="task-detail-title">
              ...
              <section className="task-people" aria-labelledby="task-people-title">
                ...
                {gateway.mode === 'live'
                  ? <div className="collaborator-list">{liveCollaborators.length ? liveCollaborators.map((person) => <div className="collaborator-chip" data-testid={`task-collaborator-${person.userId}`} key={person.userId}><strong>{person.name}</strong><button className="icon-button" type="button" disabled={!selectedIsOwner || selectedReadonly} aria-label={`Remove ${person.name}`} aria-describedby={!selectedIsOwner ? ownerOnlyReasonId : selectedReadonly ? readonlyReasonId : undefined} onClick={() => void removeLiveCollaborator(person.userId)} data-testid={`task-remove-collaborator-${person.userId}`}><Icon name="close" size={13} /></button></div>) : <p>No collaborators yet.</p>}<div className="inspector-pair"><label>Workspace member<select value={collaboratorUserId} disabled={!selectedIsOwner || selectedReadonly || memberStatus !== 'ready'} onChange={(event) => setCollaboratorUserId(event.target.value)} data-testid="task-collaborator-input"><option value="">Select a member</option>{memberOptions.filter((person) => person.id !== selectedTask.ownerId && !liveCollaborators.some((existing) => existing.userId === person.userId)).map((person) => <option key={person.id} value={person.id}>{person.name}{person.email ? ` · ${person.email}` : ''}</option>)}</select></label><button className="secondary-button" type="button" disabled={!selectedIsOwner || selectedReadonly || !collaboratorUserId} onClick={() => void addLiveCollaborator()} data-testid="task-add-collaborator">Add</button></div>{memberStatus === 'error' && <p role="alert">Workspace members could not be loaded.</p>}</div>
