            ...
            <ListInspector
              ...
              inspector={(item) => {
                ...
                if (itemSection === 'google-calendar') return <section data-testid="integration-inspector">
                  <header className="detail-header"><div><span className={`pg-integrations-status is-${calendar.status}`} data-testid="integration-status-google-calendar">{statusLabel(calendar)}</span><p>{calendar.identity ?? 'No account identity available'}{calendar.lastSyncedAt ? ` · Last synced ${calendar.lastSyncedAt}` : ''}</p></div><div className="row-actions">{calendar.status === 'connected' ? <button className="secondary-button" type="button" onClick={() => syncProvider('google-calendar')} disabled={providerSyncing !== null} data-testid="google-calendar-sync">{providerSyncing === 'google-calendar' ? 'Syncing…' : 'Sync'}</button> : <button className="secondary-button" type="button" onClick={() => openHandoff('google')} data-testid={calendar.status === 'disconnected' ? 'google-calendar-connect' : 'google-calendar-reconnect'}>{calendar.status === 'disconnected' ? 'Connect' : 'Reconnect'}</button>}</div></header>
