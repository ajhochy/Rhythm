...
export function IntegrationsPage({ route }: { route: string }) {
  ...
  const [selectedId, setSelectedId] = useSelectedId('integrationId');
  const routeSelectedId = section && supportedSections.has(section) ? `integration-${section}` : 'integration-google-calendar';
  const effectiveSelectedId = selectedId ?? routeSelectedId;
  const selectedSection = effectiveSelectedId.startsWith('integration-') ? effectiveSelectedId.slice('integration-'.length) : '';
  ...
  const providerItem = (provider: IntegrationAccount): ListInspectorItem => ({
    id: `integration-${provider.id}`,
    title: provider.name,
    subtitle: provider.identity ?? 'No account identity available',
    meta: statusLabel(provider),
    badge: provider.errorMessage,
    group: 'providers',
  });
  const integrationItems: ListInspectorItem[] = pageState === 'empty' ? [] : [
    providerItem(calendar),
    providerItem(gmail),
    providerItem(pco),
    { id: 'integration-assistant-tools', title: 'Assistant access', subtitle: 'Separate consent', meta: 'Broader Google access', group: 'tools' },
    { id: 'integration-import', title: 'AI Import', subtitle: 'Tasks, rhythms, and project templates', meta: 'Opens on request', group: 'tools' },
  ];
  const integrationError = pageState === 'server-error' || pageState === 'forbidden' || pageState === 'unavailable'
    ? <StatePanel state={pageState} onRetry={() => { if (isLive) retryLiveLoad(); else { setReceipts(loadReceipts(fixtureAccounts)); replaceState('ready'); } }} onConnect={() => openHandoff('google')} />
    : undefined;
      ...
      {unknownSection ? <div className="tool-state-panel pg-integrations-state warning" role="alert" data-testid="integration-section-not-found"><span className="pg-integrations-state-mark" aria-hidden="true">404</span><span className="eyebrow">Unknown deep link</span><h2>Integration section not found</h2><p>The requested section is not part of the Integrations fixture.</p><button className="secondary-button" type="button" onClick={() => navigate('/integrations')} data-testid="integrations-back">Back to integrations</button></div>
        : <>
          ...
          <ListInspector
              ...
              groups={[{ id: 'providers', label: 'Providers' }, { id: 'tools', label: 'Tools' }]}
              selectedId={pageState === 'empty' ? null : effectiveSelectedId}
