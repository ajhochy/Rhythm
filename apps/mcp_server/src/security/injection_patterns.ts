/**
 * injection_patterns.ts — Issue #1134
 *
 * VENDORED MIRROR — verbatim copy of
 * `apps/api_server/src/security/injection_patterns.ts` (Issue #873).
 *
 * mcp_server is a standalone commonjs package that imports nothing from
 * api_server (see docs/ai/current-plan.md #1134), so this fail-closed pattern
 * list is copied rather than imported to keep the gate inside the same
 * process that emits the tool result. If the source file changes, re-copy it
 * here and note the sync in a `docs/ai/decisions/` entry — two copies is
 * cheaper than a shared package until a third consumer needs one (YAGNI).
 *
