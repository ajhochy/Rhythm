...
const SECURITY_EXTRA = {
  _meta: {
    [RHYTHM_SECURITY_CONTEXT_META_KEY]: {
      sdkSessionId: "sdk-automations-test",
      turnId: "turn-automations-test",
      agentName: "org-optimizer",
      toolCallId: "call-automations-test",
    },
  },
};
...
describe("registerAutomationTools", () => {
  ...
  it("registers all 10 automation tools", () => {
    ...
    for (const name of EXPECTED_TOOL_NAMES) {
      expect(tools.has(name), `expected tool ${name} to be registered`).toBe(
        true,
      );
    }
  ...
  describe("rhythm_list_automations", () => {
    it("calls apiGet with /automation-rules", async () => {
      ...
      await tools.get("rhythm_list_automations")!.handler({}, SECURITY_EXTRA);
    ...
    it("filters client-side when enabled_only is true", async () => {
      ...
      const res = await tools
        .get("rhythm_list_automations")!
        .handler({ enabled_only: true }, SECURITY_EXTRA);
      const text = res.content[0].text;
      const parsed = parseFencedJson(text) as Array<{ id: string }>;
    ...
    it("does not filter when enabled_only is omitted", async () => {
      ...
      expect(parseFencedJson(res.content[0].text)).toHaveLength(2);
    });
  });
  ...
  describe("rhythm_create_automation", () => {
    ...
    it("omits optional fields when not provided", async () => {
      ...
      expect(Object.keys(body).sort()).toEqual(
        ["actionType", "name", "source", "triggerKey"].sort(),
      );
    });
  });
  ...
  describe("rhythm_preview_automation", () => {
    it("calls apiGet with preview path", async () => {
      ...
      expect(apiGet).toHaveBeenCalledWith(
        API_URL,
        API_TOKEN,
        "/automation-rules/abc-123/preview",
      );
    });
  });
  ...
  describe("rhythm_resync_automation", () => {
    it("calls apiPost with resync path", async () => {
      ...
      expect(apiPost).toHaveBeenCalledWith(
        API_URL,
        API_TOKEN,
        "/automation-rules/abc-123/resync",
        {},
      );
    });
  });
