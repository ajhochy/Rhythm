...
modelProvider + modelId optionally override the bound profile's model for this task (e.g. run a monthly report on a stronger model). Set them together or omit both to use the profile's model.`,
    {
      ...
      prompt: z
        .string()
        .describe("The prompt the agent receives when this task fires."),
      scheduleType: z.enum(["daily", "weekly", "monthly", "cron", "once"]),
      scheduledTime: z
        .string()
        .optional()
        .describe("HH:MM for daily/weekly/monthly."),
      scheduledDay: z
        .number()
        .optional()
        .describe(
          "Day of week (0-6) for weekly, or day of month (1-28) for monthly.",
        ),
      cronExpression: z
        .string()
        .optional()
        .describe('5-field cron expression e.g. "0 8 * * 1-5"'),
      runAt: z
        .string()
        .optional()
        .describe("ISO 8601 datetime for once-off tasks."),
      timezone: z
        .string()
        .optional()
        .describe("IANA timezone, default America/Los_Angeles."),
      agentConfigId: z
        .string()
        .optional()
        .describe(
          'Rhythm agent profile id to run, e.g. "AI-Trend-Researcher".',
        ),
      allowedMcps: z
        .array(z.string())
        .optional()
        .describe(
          "MCP server names allowed for this run. Omit to inherit the profile scope.",
        ),
      allowedSkills: z
        .array(z.string())
        .optional()
        .describe(
          "Skill names allowed for this run. Omit to inherit the profile scope.",
        ),
      modelProvider: z
        .string()
        .optional()
        .describe(
          'Override the profile model provider for this task (e.g. "anthropic"). Set together with modelId.',
        ),
      modelId: z
        .string()
        .optional()
    ...
    async (_args, extra) => {
      try {
        ...
        const ingress = await scanContextContentAndRecordExternalContentTaint({
          ...
          source: "scheduled-task.list",
          label: "user-authored scheduled agent tasks",
    ...
    async ({ id }: { id: string }) => {
      try {
        const result = await apiPost(
          apiUrl,
          apiToken,
          `/agent-schedules/${id}/trigger-now`,
          {},
        );
        return toolResult(JSON.stringify(result, null, 2));
      } catch (err) {
