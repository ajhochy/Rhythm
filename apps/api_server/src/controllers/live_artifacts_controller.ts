...
import type { LiveArtifactVisibility } from '../models/live_artifact';
...
export class LiveArtifactsController {
  ...
  async list(req: Request, res: Response, next: NextFunction) { try { if (req.query.type !== undefined && req.query.type !== 'html') throw AppError.badRequest('type must be html'); res.json(await repo.list(req.auth!.user.id, typeof req.query.search === 'string' ? req.query.search : undefined)); } catch (error) { next(error); } }
  async create(req: Request, res: Response, next: NextFunction) { try {
    ...
    try {
      ...
      res.status(201).json(await repo.create({ id, type: 'html', title: req.body.title.trim(), ownerUserId: actor, workspaceId, visibility: req.body.visibility === undefined ? 'private' : visibility(req.body.visibility), currentBundleRevision: 1, currentBundleHash: bundleHash, currentStateRevision: 1, currentStateHash: stateHash, declaredCapabilities: declared, updatedByUserId: actor }, collaboratorIds));
    } catch (error) { await storage.removeArtifact(id).catch(() => undefined); throw error; }
  } catch (error) { next(error); } }
  async get(req: Request, res: Response, next: NextFunction) { try { const artifact = await this.readable(req); if (artifact.deletedAt) return this.tombstone(res); res.json({ ...artifact, state: await storage.readState(artifact.id, artifact.currentStateHash) }); } catch (error) { next(error); } }
  async render(req: Request, res: Response, next: NextFunction) { try { const artifact = await this.readable(req); if (artifact.deletedAt) return this.tombstone(res); const bundle = await storage.readBundle(artifact.id, artifact.currentBundleHash); const nonce = randomBytes(16).toString('base64'); const css = bundle.css.replace(/<\/style/gi, '\\3c /style'); const js = bundle.js.replace(/<\/script/gi, '<\\/script'); const html = bundle.html.replace(/<meta\b[^>]*\bhttp-equiv\s*=\s*(?:"refresh"|'refresh'|refresh)[^>]*>/gi, ''); res.set({ 'Content-Type': 'text/html; charset=utf-8', 'Content-Security-Policy': `sandbox allow-scripts; default-src 'none'; script-src 'nonce-${nonce}'; style-src 'nonce-${nonce}'; connect-src 'none'; form-action 'none'; base-uri 'none'; frame-src 'none'; object-src 'none'; frame-ancestors 'none'`, 'X-Content-Type-Options': 'nosniff', 'Referrer-Policy': 'no-referrer' }); res.send(`<!doctype html><html><head><meta charset="utf-8"><style nonce="${nonce}">${css}</style></head><body>${html}<script nonce="${nonce}">${js}</script></body></html>`); } catch (error) { next(error); } }
  async patch(req: Request, res: Response, next: NextFunction) { try { const artifact = await this.owner(req); const title = req.body?.title; if (title !== undefined && (typeof title !== 'string' || !title.trim())) throw AppError.badRequest('title must be non-empty'); const declared = req.body?.declaredCapabilities; if (declared !== undefined && (!Array.isArray(declared) || declared.some((item) => !capabilities.includes(item)))) throw AppError.badRequest('unsupported capability'); res.json(await repo.metadata(artifact.id, title?.trim(), req.body?.visibility === undefined ? undefined : visibility(req.body.visibility), declared, req.auth!.user.id)); } catch (error) { next(error); } }
