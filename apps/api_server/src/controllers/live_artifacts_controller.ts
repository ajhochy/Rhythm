...
export class LiveArtifactsController {
  ...
  async render(req: Request, res: Response, next: NextFunction) { try { const artifact = await this.readable(req); if (artifact.deletedAt) return this.tombstone(res); const bundle = await storage.readBundle(artifact.id, artifact.currentBundleHash); const nonce = randomBytes(16).toString('base64'); const css = bundle.css.replace(/<\/style/gi, '\\3c /style'); const js = bundle.js.replace(/<\/script/gi, '<\\/script'); const html = bundle.html.replace(/<meta\b[^>]*\bhttp-equiv\s*=\s*(?:"refresh"|'refresh'|refresh)[^>]*>/gi, ''); const bootstrap = `(()=>{const n=${JSON.stringify(nonce)},p=new Map;let i=0;const rhythm=Object.freeze({request:(method,params)=>new Promise((resolve,reject)=>{const id=String(++i);p.set(id,{resolve,reject});RhythmBridge.postMessage(JSON.stringify({id,method,params,nonce:n}));})});const blocked=r=>rhythm.request("host.blocked",r);addEventListener("submit",()=>blocked("form"),true);addEventListener("click",e=>{const t=e.target;if(t?.matches?.('input[type=file]'))blocked("file");if(t?.closest?.('a[download]'))blocked("download");},true);Object.defineProperty(window,"rhythm",{value:rhythm,writable:false,configurable:false});Object.defineProperty(window,"__rhythmHostResponse",{value:function(x){if(!x||x.n!==n||typeof x.id!=="string")return;const q=p.get(x.id);if(!q)return;p.delete(x.id);x.ok?q.resolve(x.data):q.reject(x.error);},writable:false,configurable:false});})();`; const documentPolicy = `default-src 'none'; script-src 'nonce-${nonce}'; style-src 'nonce-${nonce}'; connect-src 'none'; form-action 'none'; base-uri 'none'; frame-src 'none'; object-src 'none'`; res.set({ 'Content-Type': 'text/html; charset=utf-8', 'Content-Security-Policy': `sandbox allow-scripts; ${documentPolicy}; frame-ancestors 'none'`, 'X-Content-Type-Options': 'nosniff', 'Referrer-Policy': 'no-referrer' }); res.send(`<!doctype html><html><head><meta http-equiv="Content-Security-Policy" content="${documentPolicy}"><meta charset="utf-8"><style nonce="${nonce}">${css}</style></head><body>${html}<script nonce="${nonce}">${bootstrap}</script><script nonce="${nonce}">${js}</script></body></html>`); } catch (error) { next(error); } }
