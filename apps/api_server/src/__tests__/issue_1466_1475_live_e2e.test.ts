...
describeLive('issues #1466/#1475 live HTTP behavior', () => {
  ...
  const newerRootIds = Array.from({ length: 100 }, (_, index) => `issue-1466-newer-root-${index}-${marker}`);
  ...
  beforeAll(() => {
    ...
    const insertSession = db.prepare(
       `INSERT INTO agent_sessions
         (id, agent_kind, status, cwd, name, created_at, updated_at, last_activity_at,
           parent_session_id, owner_user_id, category)
        VALUES (?, 'claude-code', 'idle', ?, ?, ?, ?, ?, ?, ?, 'chat')`,
    );
    const oldTimestamp = '2000-01-01T00:00:00.000Z';
    insertSession.run(rootId, sandboxDir, `Issue 1466 root ${marker}`, oldTimestamp, oldTimestamp, '2099-01-01T00:00:00.000Z', null, userId);
    childIds.forEach((childId, index) => {
      const timestamp = new Date(Date.UTC(2026, 0, 1, 0, 0, index)).toISOString();
      insertSession.run(childId, sandboxDir, `Issue 1466 child ${index}`, timestamp, timestamp, timestamp, rootId, userId);
    });
    newerRootIds.forEach((newerRootId, index) => {
      const timestamp = new Date(Date.UTC(2026, 1, 1, 0, 0, index)).toISOString();
      insertSession.run(newerRootId, sandboxDir, `Issue 1466 newer root ${index}`, timestamp, timestamp, timestamp, null, userId);
    });
  });
  ...
  afterAll(async () => {
    try {
      ...
      if (db) {
        db.prepare('DELETE FROM agent_sessions WHERE owner_user_id = ?').run(userId);
  ...
  it('issue-1466: a recently active old root remains top-level with all 101 children nested', async () => {
    // Regression caught: created_at ordering drops this deliberately old root
    // behind 100 newer roots despite its newest last_activity_at.
