...
/**
 * Reduce raw auth.json content to a stable ACCOUNT-IDENTITY fingerprint that
 * ignores volatile per-refresh fields.
 *
 * Why (#856 follow-up, found in manual smoke): opencode rotates the `access`
 * token (and its `expires`) on a routine refresh while the account is
 * unchanged, and persists that back to auth.json. Comparing raw bytes then
 * sees every access-token rotation — including the running engine refreshing
 * its OWN token — as a "change" and bounces the engine spuriously (observed:
 * one redundant engine restart right after boot). The bounce we actually want
 * is on a genuine account/credential-source switch.
 *
 * The fingerprint keeps, per provider, only the fields that change on a real
 * re-auth: `type`, the oauth `refresh` token, and the api `key` — dropping the
 * volatile `access`/`expires`. A pure access-token rotation → identical
 * fingerprint → no bounce; an account switch (new refresh token) or api-key
 * change → different fingerprint → bounce. On unparseable content we fall back
 * to the raw string so we never silently miss a change.
 */
export function authIdentityFingerprint(content: string | null): string {
  if (content === null) return ' null';
  let parsed: unknown;
  try {
    parsed = JSON.parse(content);
  } catch {
    return content; // fall back to raw comparison on non-JSON
  }
  ...
  // property *allowlist*, which would strip the nested type/refresh/key fields.
  const out: Record<string, unknown> = {};
  for (const provider of Object.keys(src).sort()) {
    const entry = src[provider];
    if (!entry || typeof entry !== 'object') {
      out[provider] = entry;
      continue;
    }
    const e = entry as Record<string, unknown>;
    // Only identity-bearing fields; access/expires are intentionally excluded.
    out[provider] = { type: e.type, refresh: e.refresh, key: e.key };
  }
 ...
 * engine.
 *
 * Comparison is on the account-identity fingerprint (see
 * {@link authIdentityFingerprint}), NOT raw bytes, so a routine access-token
 * refresh does not trigger a bounce.
 *
 * Rules (acceptance criteria for #856):
 *   - Unchanged identity → never reload (`unchanged`).
...
export function decideReload(
  ...
  if (
    authIdentityFingerprint(previous.content) ===
    authIdentityFingerprint(next.content)
  ) {
