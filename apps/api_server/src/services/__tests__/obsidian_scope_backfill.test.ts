...
describe('backfillObsidianReadScope (real DB)', () => {
  it('grants obsidian to selectable array + object-map scopes; preserves entries; idempotent', () => {
    ...
    // all selectable with NULL scope, plus the Config Doctor profile (explicit
    // empty-array scope, '[]') and the Rhythm Setup profile (#911, scope
    // '["rhythm"]') — both array-scoped and selectable, so both ARE eligible
    // for this backfill same as any other array-scoped row (see the seed
    // comment in migrations.ts). Use unique ids and assert on our own rows
    // for everything else.
