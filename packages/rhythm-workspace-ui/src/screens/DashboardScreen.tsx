...
function StatePanel({ state, onRetry, onEmpty, canWrite }: { state: Exclude<DashboardSurfaceState, 'ready'>; onRetry(): void; onEmpty(): void; canWrite: boolean }) {
  if (state === 'loading') {
...
function TaskEntry({ task, onInspect, onToggle, readonly }: { task: RhythmDashboardTask; onInspect(task: RhythmDashboardTask): void; onToggle(task: RhythmDashboardTask): void; readonly: boolean }) {
  return (
    <article className="task-entry">
      <button className="task-toggle" type="button" disabled={readonly} aria-label={`${task.status === 'done' ? 'Reopen' : 'Complete'} ${task.title}`} onClick={() => onToggle(task)} data-testid={`task-toggle-${task.id}`}><span aria-hidden="true">{task.status === 'done' ? '✓' : '○'}</span></button>
...
function ProjectStepEntry({ step, onInspect, onToggle, readonly }: { step: RhythmDashboardProjectStep; onInspect(step: RhythmDashboardProjectStep): void; onToggle(step: RhythmDashboardProjectStep): void; readonly: boolean }) {
  return (
    <article className="task-entry">
      <button className="task-toggle" type="button" disabled={readonly} aria-label={`${step.status === 'done' ? 'Reopen' : 'Complete'} ${step.title}`} onClick={() => onToggle(step)} data-testid={`project-step-toggle-${step.id}`}><span aria-hidden="true">{step.status === 'done' ? '✓' : '○'}</span></button>
      ...
      <section className="page-shell pg-dashboard" aria-busy={surfaceState === 'loading'}>
        <header className="dashboard-toolbar">
          ...
          <div className="dashboard-header-actions">
            ...
            <HeaderTaskAction onClick={() => canWrite && setCreateOpen(true)} disabled={!canWrite || !isContentVisible || mutationPending} testId="dashboard-header-add-task" />
        ...
        <div className="dashboard-scroll">
          {!isContentVisible && <StatePanel state={surfaceState} onRetry={() => void load()} onEmpty={() => canWrite && setCreateOpen(true)} canWrite={canWrite} />}
          {isContentVisible && summary && (
            <>
              {!canWrite && <p className="inspector-prerequisite" role="status" data-testid="dashboard-readonly-explanation"><strong>Read-only workspace</strong><span>Your host has not granted Dashboard write access. You can inspect work and ask Hermes, but changes are unavailable.</span></p>}
              ...
              <section className="planning-section" aria-labelledby="planning-title">
                ...
                <div className="planning-grid">
                  <article className="planning-card wide" data-testid="planning-past-due"><div className="list-head"><h3>Past due · {pastDueTasks.length}</h3></div>{pastDueTasks.map((task) => <TaskEntry key={task.id} task={task} readonly={!canWrite || mutationPending} onInspect={setSelectedTask} onToggle={(item) => void toggleTask(item)} />)}{!pastDueTasks.length && <p className="empty-copy">Nothing overdue.</p>}</article>
                  ...
                  <article className="planning-card" data-testid="planning-today"><div className="list-head"><h3>Today · {todayTasks.length}</h3></div>{todayTasks.map((task) => <TaskEntry key={task.id} task={task} readonly={!canWrite || mutationPending} onInspect={setSelectedTask} onToggle={(item) => void toggleTask(item)} />)}</article>
                  <article className="planning-card" data-testid="planning-week"><div className="list-head"><h3>This week · {weekTasks.length}</h3></div>{weekTasks.map((task) => <TaskEntry key={task.id} task={task} readonly={!canWrite || mutationPending} onInspect={setSelectedTask} onToggle={(item) => void toggleTask(item)} />)}{!weekTasks.length && <p className="empty-copy">No open tasks later this week.</p>}</article>
                  <article className="planning-card" data-testid="planning-project-steps"><div className="list-head"><h3>Project on deck</h3><span>{projectSteps.filter((step) => step.status === 'open').length}</span></div>{projectSteps.map((step) => <ProjectStepEntry key={step.id} step={step} readonly={!canWrite || mutationPending} onInspect={setSelectedStep} onToggle={(item) => void toggleStep(item)} />)}{!projectSteps.length && <p className="empty-copy">All project steps are complete.</p>}</article>
                  <article className="planning-card wide" data-testid="planning-unscheduled"><div className="list-head"><h3>Unscheduled · {unscheduledTasks.length}</h3></div>{unscheduledTasks.map((task) => <TaskEntry key={task.id} task={task} readonly={!canWrite || mutationPending} onInspect={setSelectedTask} onToggle={(item) => void toggleTask(item)} />)}{!unscheduledTasks.length && <p className="empty-copy">Every open task has a date.</p>}</article>
                </div>
              </section>
            </>
          )}
        ...
        <FocusDialog open={Boolean(selectedTask)} onClose={() => setSelectedTask(null)} title="Task details" description="Inspect or update the selected task." testId="task-inspector" wide>
          <form className="inspector-form" onSubmit={saveTask}>
            <fieldset disabled={!canWrite || mutationPending}><label>Task title<input disabled={!canWrite || mutationPending} name="inspectorTitle" defaultValue={selectedTask?.title ?? ''} data-autofocus data-testid="task-inspector-title" /></label>
            <label>Notes<textarea disabled={!canWrite || mutationPending} name="inspectorNotes" defaultValue={selectedTask?.notes ?? ''} rows={4} data-testid="task-inspector-notes" /></label>
            <div className="inspector-pair">
              <label>Scheduled date<input disabled={!canWrite || mutationPending} name="inspectorScheduledDate" type="date" defaultValue={selectedTask?.scheduledDate ?? ''} data-testid="task-inspector-scheduled" /></label>
              <label>Due date<input disabled={!canWrite || mutationPending} name="inspectorDueDate" type="date" defaultValue={selectedTask?.dueDate ?? ''} data-testid="task-inspector-due" /></label>
            </div>
            <section className="inspector-collaborator" aria-label="Task collaborator">
              ...
              {selectedTask?.collaboratorId
                ? <button className="text-danger-button" type="button" disabled={!canWrite || mutationPending} onClick={() => void updateInspectorCollaborator(null)} data-testid="task-inspector-collaborator-remove">Remove</button>
                : <button className="secondary-button" type="button" disabled={!canWrite || mutationPending || !members[0]} onClick={() => members[0] && void updateInspectorCollaborator(members[0].id)} data-testid="task-inspector-collaborator-add">Add {members[0]?.name ?? 'collaborator'}</button>}
            </section>
            <footer><button className="secondary-button" type="button" onClick={() => setSelectedTask(null)} data-testid="task-inspector-cancel">Cancel</button><button className="primary-button" type="submit" disabled={!canWrite || mutationPending} data-testid="task-inspector-save">Save changes</button></footer></fieldset>
          </form>
        </FocusDialog>
        ...
        <FocusDialog open={Boolean(selectedStep)} onClose={() => setSelectedStep(null)} title="Project step details" description={project ? `${project.title} · project step` : 'Project step'} testId="project-step-inspector" wide>
          <form className="inspector-form" onSubmit={(event) => { event.preventDefault(); if (canWrite) setSelectedStep(null); }}><fieldset disabled={!canWrite || mutationPending}>
            ...
            <footer><button className="secondary-button" type="button" onClick={() => setSelectedStep(null)} data-testid="project-step-cancel">Cancel</button><button className="primary-button" type="submit" data-testid="project-step-save">Save changes</button></footer></fieldset>
          </form>
        </FocusDialog>
      </section>
    </ScreenRoot>
