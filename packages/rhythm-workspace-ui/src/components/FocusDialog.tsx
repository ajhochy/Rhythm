// Ported verbatim (host-neutral already — no Electron/session/bearer coupling) from
// apps/web/src/components/FocusDialog.tsx: a focus-trapped, Escape-to-close, focus-restoring
// dialog. Used by every screen's create/edit/collaborator/delete dialogs.
...
export function FocusDialog({
  ...
  open: boolean; title: string; description?: string; onClose(): void; children: ReactNode; testId: string; wide?: boolean;
}) {
  ...
  useLayoutEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      // Non-null: guarded by the length check above, so index 0 and length-1 always exist.
