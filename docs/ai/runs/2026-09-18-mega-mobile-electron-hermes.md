...
## Final implementation CI and signed candidate

Implementation70bcf2c9 passed Server CI(6185/253/0), Fork CI including SDK currency, and Mobile CI(70 passed/1 skipped/1 flaky). Final ARM qualification35454150623 signed/notarized/uploaded without release publication. Downloaded and separately installed bundle passed local trust checks; Finder/Get Info and safe native old-server rejection passed. Nine authenticated steps and two remaining icon surfaces are still blocked. The stopped signed candidate is retained in user Applications; protected apps/services remain running. [Final qualification, remaining gates and decisions](2026-09-19-final-source-qualification.md) records source/hash, final Intel status and cleanup. Historical failures above remain valid observations.
