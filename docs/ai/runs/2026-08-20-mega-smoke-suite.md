...
- The React gateway now mirrors Flutter's two-server auth boundary: the signed-in cloud bearer remains on Synology requests but is stripped from local API requests, where an invalid Authorization header intentionally defeats the `AGENT_LOCAL` bypass. The focused boundary regression passed and the complete gateway contract is `9/9` with TypeScript clean.
