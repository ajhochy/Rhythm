...
pr: none (landed directly on the branch, per AJ)
issues: []
...
| Empty `/agents/models` + `/opencode/models` | Red herring, and not even a defect: those endpoints return `[]` only when called without their required `?agentId=` / `?provider=` parameter. Tier resolution filters on `listAuthedProviders()` (the credential store) regardless, never the model catalog. |
...
- **Landed directly on `mega/2026-09-18-mobile-electron-hermes`, no PR.** AJ's explicit call: that is
  the branch his running "Rhythm Mega Desktop Candidate.app" is built from (bundled
  `apps/electron/src` verified byte-identical to 3641f303), so the work has to be there for the
  running app to pick it up. The earlier draft PR #1549 was closed as superseded. This overrides the
  usual draft-PR-only rule for this run. `main` was neither pushed to nor merged.
- **The `apps/api_server` half is NOT Electron-specific.** The auth-hop fix benefits the Flutter
  client identically; it lives on the mega branch only because that is where the Electron half has
  to go. Worth cherry-picking to `main` when the mega line merges.
- **The "empty model catalog" lead was a false alarm — withdrawn.** `GET /agents/models` and
  `GET /opencode/models` return `[]` only when called without parameters; that is by design.
  `/agents/models?agentId=claude-code` returns the full Anthropic list and
  `/agents/models/catalog` shows every row `authorized: true`. The catalog is healthy and authed,
  and there is no stale-catalog failure mode.
  Independently verified while the theory was still live, and worth recording because it rules the
  catalog out of the cascade entirely: `opencode_client_service.listAuthedProviders()` reads
  `this.authStore` (the credential store) and consults the engine catalog *only* to add keyless
  providers, while `DEFAULT_MODEL_BY_PROVIDER` hard-codes the destination model ids. So
  `resolveAuthedFallbackChain` yields team-claude → personal-claude → codex → gemini →
  openrouter-free regardless of catalog state. If the chain ever resolves to too few tiers, that is
  a bug in the chain logic, not a catalog problem.
- **Live state no longer reproduces the bug.** Both `team` and `personal` now report `ok`, so the
  unauthenticated-account path can only be exercised from fixtures or a forced status — a passing
  live run proves nothing about the auth hop. The 6 unit tests are the real evidence.
