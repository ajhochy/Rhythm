...
Latest result: actual Desktop is open in the signed local mega candidate, with the requested outer header removed. Native read/draft/tools suite 6/6, real restart draft proof and API/engine reuse passed. Full distribution, borrowed standalone coexistence and original mega hosted/device gates remain open. Earlier checkpoints below are historical; they are not current blockers.
