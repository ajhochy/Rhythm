...
## Subsequent device and relay repair

The later C5 failure is now diagnosed and repaired in the disposable test only: native Keychain authorization blocked renderer progress; a test-only mock Keychain switch preserves the real endpoint/read/disposal assertions. Root rerun passed1/1. Candidate95306 subsequently restored login/signer/runtime and actual Hermes Desktop in a2/2 native check; its relay now connects the physical phone. See [current receipt](2026-09-19-connected-phone-relay.md) for exact results and remaining failures.
