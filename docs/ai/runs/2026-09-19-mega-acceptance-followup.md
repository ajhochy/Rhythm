...
The final implementation commit70bcf2c9 synchronizes the Mobile/server contract with the generated SDK while explicitly denying raw `mcp.tools` access to Mobile. Server CI35454145321 passed6,185 tests/253 skipped/zero failed, plus Postgres bootstrap/build/API smoke/safety smoke. Mobile CI35454145315 passed overall with70 Playwright passes/1 skip/1 known flaky retry. Fork CI35454145318 passed generated-artifact currency. Earlier failures remain above.

Final-source qualification35454150623 produced the signed/notarized ARM artifact. Local codesign, Gatekeeper and stapled ticket checks passed before and after separate installation; native Finder/Get Info and safe old-server rejection passed on that exact candidate. Dock and Command-Tab remain unverified. The candidate was quit, protected services remained healthy, and owned download/extraction/unauthenticated profile copies were removed. The signed candidate is retained separately for review. See [final source qualification](2026-09-19-final-source-qualification.md) for archive hash, source, CI, Intel status, screenshots and final decisions. Fork remote head9c8dcf4230 adds evidence-only metadata; its implementation remainsa285b3651c.
