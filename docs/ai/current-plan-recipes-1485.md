...
**Status:** PR #1490 second-pass Fable review repairs incorporated. AJ's decisions are settled. Docs only; implementation has not started.
**Source baseline:** file paths and line citations target branch `plan/recipes-1485` at `4dff101c` (2026-08-27).
...
- Approval redemption first rejects a supplied token from a clean consuming session (`external_content_security_service.ts:511-518`), then enforces cross-session (`:549-550`), bound profile/agent (`:552-553`), and stale-taint (`:570-575`) guards. Before token redemption, root `bypassPermissions` may authorize without a token (`:521-528`), and unattended scheduled auto-approval has a parallel predicate (`:300-308`, `:533-540`). Current TTL is ten minutes (`:250`, `:496`). `decideWithNonce` queues continuation whenever `session_id` is non-null (`agent_approvals_repository.ts:158-176`).
...
- Optimizer recipe generation has its own legacy compiler (`recipe_generator.ts:119-143`) and creates cookbook rows programmatically (`:398`); proposal wiring is registered in `org_proposal_appliers_wiring.ts`.
...
S1a and S1b expose a neutral `legacy` classification only. They withhold `explicit_upgrade_required` through backend execution because validation alone is not an upgrade path. S5 may expose it only together with an editor-backed explicit conversion that produces a machine-valid workflow definition, previews diagnostics, and leaves the legacy row byte-identical until the user saves. This avoids an alarming dead-end state for 10–15 production users. Workflow create/update validates before persistence. Runs snapshot canonical definition and input hashes; no recipe revision-history subsystem is added.
...
A stage uses a two-phase session binding. The first `onSessionCreated(sessionId)` call atomically writes a **provisional authorization binding** for `(runId, stageExecutionId, attemptId)` before engine work starts and before the agent can call completion. The callback also durably marks the session as workflow-owned in nullable `agent_sessions.workflow_run_id` and `agent_sessions.workflow_stage_execution_id` columns. While `run()` is active, the completion endpoint authorizes only that known provisional session and records any valid completion as provisional/uncommitted; it cannot advance the stage.

`AgentRunResult.sessionId` is the final durable binding after `run()` resolves. It must equal the sole provisional session. A second `onSessionCreated` call, a returned result ID that differs, or any dispatch that produces more than one session atomically invalidates/rejects provisional completion and fails the attempt closed. Only after exact equality may the runner idempotently commit the completion and transition. Attempt-state and supersession fences reject late completion before both provisional recording and final commit, so cancellation, retry, or a newer attempt cannot resurrect work. This resolves escalation/session selection and mid-run completion authorization with one invariant rather than treating the result ID as available before the call occurs.

Observed routing comes solely from `agent_session_messages.info_json → providerID/modelID` for `role='assistant'`, joined through the final bound `result.sessionId`. Require at least one assistant message; zero observed values, a mismatch, or mixed provider/model values fail closed. Mixed values are possible because mid-run cross-provider re-dispatch already exists (`opencode_stream_bridge.ts:1487-1489`). `agent_sessions.provider_id/model_id` and configured values are never accepted as observed evidence; there is no live-engine fallback in the product contract.
...
`rhythm_complete_workflow_stage` calls a local API completion endpoint with bearer authentication **and** the engine-signed trusted call. The MCP handler forwards `currentTrustedSecurityCall()` / `TrustedSecurityContext`; the API verifies tool name and exact argument hash, then calls an exported `ExternalContentSecurityService` authorization method that internally applies the module-private known-session check and returns the durable session identity. The endpoint requires that identity to equal the active attempt's provisional binding before recording an uncommitted completion. No binding, another stage/session, stale/superseded attempt, or cancelled run is rejected before idempotency; final commit additionally waits for `run()` to return the same sole session. Thus a coder cannot post its own review verdict, and a valid gate can complete during its own run without racing an unavailable result binding.
...
AJ accepted relaxation of clean-consuming-session, cross-session, bound-profile/agent, and stale-taint redemption guards **only for rows explicitly discriminated as `approval_kind='workflow'` in the existing approval model**. `consumeApproval` may select the workflow branch only after loading that discriminator and proving its exact active run/stage/attempt binding; the workflow branch explicitly replaces all four comparisons rather than relying on their current ordering. This is a row discriminator, not a new security binding kind or parallel table. The existing session-kind branch keeps its guard expressions and errors byte-for-byte because they protect email, messaging, calendar, PCO, PR creation, and every other outbound integration from injected-content replay.

Workflow approvals retain signature, nonce, status, exact action/payload digest, bound workflow run/stage/attempt, expiry, and atomic `consumedAt`. `getWorkflowApprovalTtlMs()` reads `RHYTHM_WORKFLOW_APPROVAL_TTL_MS` fresh, accepts only a positive integer millisecond duration, and falls back to `86_400_000` (24 hours); only workflow approval creation uses it. Session approval creation continues to use the unchanged `APPROVAL_TTL_MS = 10 * 60 * 1000`. The accepted tradeoff is that a workflow approval may be redeemed after the originating session/taint turn or bound agent changes; exact active workflow identity and payload binding replace those four guards, increasing replay exposure if workflow binding is wrong. Completion authorization and single-use consumption therefore fail closed. See `docs/ai/decisions/2026-08-26-recipe-workflow-approval-guards.md`.

Workflow stage sessions are never interactive or scheduled authorization shortcuts. The shared `hasExplicitInteractiveApprovalBypass` predicate returns false immediately for any workflow-marked session or workflow-marked ancestor, even when the root has `bypassPermissions` plus `approvalBypassExplicit`. The shared `isUnattendedAutoApproveSession` predicate likewise returns false for any workflow-marked session even if `scheduledTaskId` is present. These are single negative checks in the shared predicates, not caller-by-caller guards; every existing unmarked session keeps its current behavior.
...
| `apps/api_server/src/repositories/agent_sessions_repository.ts` | Persist workflow stage markers used by shared approval-bypass exclusions and provisional binding. |
...
| `apps/api_server/src/services/agent_runner.ts` | Public escalation suppression; workflow marker option and provisional callback; conditional async start; cancellation settlement/slot release. **Highest-fan-in service: impact analysis required before edit.** |
...
| `apps/api_server/src/services/external_content_security_service.ts` | Exported trusted-session authorization method; kind-scoped workflow redemption/TTL; shared workflow exclusions from interactive/scheduled bypass; session path unchanged. |
...
| **S3a — Repository, runner, transitions, budgets, cancellation** | workflow repo/runner, session repo/DB, controller/routes, DTO tests | Nonconsequential workflows start pending; first callback persists one workflow-marked provisional session before engine work; completion stays provisional; final result must equal that sole session before idempotent commit; second/mismatched sessions fail closed; one-level fan-out/all join; closed verdict transitions; observed provider requires nonempty uniform assistant metadata; all caps; restart; flag-off terminalization; cancellation settles run and releases global slot; DTO exactly matches plan. | S0 mode + S1b; dispatch portion uses S2. | Serial provisional/final binding, mid-run completion, second-session/escalation, late/superseded completion, transition/race/provider/fan-out/budget/restart/flag/cancel/slot/DTO tests; `npm test`; `tsc --noEmit`; sandbox lifecycle run note. |
| **S3b — Approval, taint, capability, completion authorization** | workflow controller/MCP tool; security service; approval/session repos; policy tests | Consequential stages cannot run in S3a; exported service authorization resolves only a known trusted caller session; completion requires its provisional active-attempt binding and final equality before commit; workflow-kind explicitly replaces clean-session, cross-session, bound-agent, and stale-taint guards only after exact active workflow binding; workflow markers make both shared interactive and scheduled bypass predicates false; unmarked/session approvals are byte-identical; workflow-only TTL is configurable; no continuation double-advance; policy/capability floor and sensitive ledger policy hold. | S3a + S1b. | Forgery/coder-self-verdict/unknown-session/provisional-final mismatch/second-session/late-superseded replay/clean-session/cross-session/bound-agent/stale-taint/workflow-marker bypass/expiry/mixed-kind/session-regression/double-advance/taint/grant/retention tests; live trusted-call behavioral test in sandbox. |
...
| **S5 — Flutter editor/monitor** | Flutter models/data/view/widgets/tests | Six controls below work; no raw JSON/drag canvas/new dependency; inline diagnostics; neutral legacy state remains runnable; explicit upgrade state appears only with an editor-backed conversion that validates a machine-valid target and does not mutate legacy data before save; generated flow; real view starts/monitors DTO and approval across restart. | S1b + stable S3 DTO. Visualization demo may use S4 fixture but editor is not gated on S4. | format; analyze; feature tests; macOS manual smoke legacy conversion/create/edit/view/run/restart/approve/reject/terminal URL. |
...
| H1 second pass: all approval guards and automation bypasses | Baseline; approvals; file map; S3b; approval decision record |
| H2 second pass: completion binding race | Provider/session binding; completion authorization; file map; S3a/S3b |
| Citation/state corrections | Header/baseline; compatibility; extraction decision attribution; S1b/S5 |
