...
- [x] Source and package diff validation
  ...
  - What to verify: `apps/mcp_server/package.json` is version `0.3.0`; both new tool names are registered; `rhythm_add_rhythm_step` accepts `rhythm_id`, `title`, optional `day_of_week`, and optional `sort_order`; `day_of_week` is converted to numeric `dayOfWeek`; new steps omit `id`; `rhythm_delete_rhythm_step` accepts `rhythm_id` and `step_id` and returns an MCP error if the step is absent.
  ...
  - ✅ Evidence: `/Users/ajhochhalter/.clideck/plugins/workflow/workflows/wf-2026-05-07-2yo8tk/evidence/source-diff.txt`
    - `apps/mcp_server/package.json` shows version `0.3.0`.
    - Source diff shows `rhythm_add_rhythm_step`, `rhythm_delete_rhythm_step`, `parseDayOfWeek`, `dayOfWeek: dayOfWeekNum ?? null`, and missing-step `toolError`.
    - Diff inspection was scoped to `apps/mcp_server/package.json` and `apps/mcp_server/src/tools/rhythms.ts`.
...
- [x] MCP tool discovery over stdio
  ...
  - ✅ Evidence: `/Users/ajhochhalter/.clideck/plugins/workflow/workflows/wf-2026-05-07-2yo8tk/evidence/mcp-tools-list.log`
    - MCP `tools/list` returned `rhythm_add_rhythm_step` with required `rhythm_id`, `title` and optional `day_of_week`, `sort_order`.
    - MCP `tools/list` returned `rhythm_delete_rhythm_step` with required `rhythm_id`, `step_id`.
    - Script printed `ASSERTIONS PASSED: rhythm step tools are discoverable with expected schemas.`
...
- [x] MCP add/delete rhythm step behavior against a mock Rhythm API
  ...
  - ✅ Evidence: `/Users/ajhochhalter/.clideck/plugins/workflow/workflows/wf-2026-05-07-2yo8tk/evidence/mcp-mock-api.log`
    ...
    - Script printed `ASSERTIONS PASSED: add/delete rhythm step behavior matched expected API calls and MCP responses.`
...
- [ ] Published package visibility
  ...
  - ❌ Evidence: `/Users/ajhochhalter/.clideck/plugins/workflow/workflows/wf-2026-05-07-2yo8tk/evidence/npm-view.log`
    ...
    - `npm view` exited 0, so the package exists, but the newly bumped version has not been published.
